...
func TestSchemaMigrations(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "Should up tenant/00014_add_more_enum_check_constraints.sql",
			downgrade: false,
			target:    db.TenantTarget,
			version:   14,
			assertMigration: func(t *testing.T) func(con *multitenancy.DB) error {
		...
		{
			name:      "Should down tenant/00014_add_more_enum_check_constraints.sql",
			downgrade: true,
			target:    db.TenantTarget,
			version:   14,
		},
...
// TestMoreEnumCheckConstraintDrift asserts the IN(...) lists in 00014 match
// the Go enum slices. OAPI-generated enums (KeyType, KeyAlgorithm, Wrapping*)
// are intentionally not duplicated here — their source of truth is the spec.
func TestMoreEnumCheckConstraintDrift(t *testing.T) {
	const migrationFile = "../../migrations/tenant/schema/00014_add_more_enum_check_constraints.sql"
	...
	cases := []struct {
		...
		{
			name:       "workflows.parameters_resource_type",
			constraint: "chk_workflows_parameters_resource_type",
			want:       toStrings(model.WorkflowParametersResourceTypes),
		},
		{
			name:       "systems.type",
			constraint: "chk_systems_type",
			want:       toStrings(model.SystemTypes),
		},
		{
			name:       "certificates.state",
			constraint: "chk_certificates_state",
			want:       toStrings(model.CertificateStates),
		},
		{
			name:       "certificates.purpose",
			constraint: "chk_certificates_purpose",
			want:       toStrings(model.CertificatePurposes),
		},
	}
