...
type TenantOperator struct {
	...
	sessionManagerClient oidcmappinggrpc.ServiceClient
...
func NewTenantOperator(
	...
	if sessionManagerClient == nil {
		return nil, oops.Errorf("sessionManagerClient is nil")
	}

	r := sql.NewRepository(db)

	gm := manager.NewGroupManager(r, catalog)
	tm := manager.NewTenantManager(r)
...
func (o *TenantOperator) applyOIDC(ctx context.Context, tenantID string, cfg OIDCConfig) error {
	return o.repo.Transaction(ctx, func(ctx context.Context, tx repo.Repo) error {
		success, err := tx.Patch(ctx, &model.Tenant{
			ID:        tenantID,
		...
		resp, err := o.sessionManagerClient.ApplyOIDCMapping(ctx, &oidcmappinggrpc.ApplyOIDCMappingRequest{
			TenantId:   tenantID,
			Issuer:     cfg.Issuer,
			JwksUris:   cfg.JwksUris,
...
func (o *TenantOperator) handleBlockTenant(
	...
	resp, err := o.sessionManagerClient.BlockOIDCMapping(ctx, &oidcmappinggrpc.BlockOIDCMappingRequest{
...
func (o *TenantOperator) handleUnblockTenant(
	...
	resp, err := o.sessionManagerClient.UnblockOIDCMapping(ctx, &oidcmappinggrpc.UnblockOIDCMappingRequest{
		TenantId: tenantProto.GetId(),
	})
	//nolint:nilerr
	if err != nil {
...
func (o *TenantOperator) handleTerminateTenant(
	...
	resp, err := o.sessionManagerClient.RemoveOIDCMapping(ctx, &oidcmappinggrpc.RemoveOIDCMappingRequest{
		TenantId: tenantProto.GetId(),
	})
	//nolint:nilerr
	if err != nil {
...
func (o *TenantOperator) sendTenantUserGroupsToRegistry(ctx context.Context, tenantID string) (bool, error) {
	...
	ctx = log.InjectGroups(ctx, groups)
...
func extractOIDCConfig(properties map[string]string) (OIDCConfig, error) {
	...
	var issuer, jkwsUris, audiences string
	...
	cfg := OIDCConfig{
		Issuer:               issuer,
		JwksUris:             parseCommaSeparatedValues(jkwsUris),
