...
func createContext(t *testing.T) context.Context {
	ctx := t.Context()
	ctx, _ = cmkcontext.InjectInternalClientData(ctx, constants.InternalTenantProvisioningRole)
	return ctx
}
...
func createManagers(
	...
	um := manager.NewUserManager(authzRepo, nil, cmkAuditor)
...
func TestTenantOperatorTracing(t *testing.T) {
	responder := respondertest.NewResponder()
	target := orbital.TargetOperator{
		Client: responder,
	}
