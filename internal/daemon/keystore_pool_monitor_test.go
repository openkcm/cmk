...
func getKeystorePoolFillMetric(t *testing.T, reader *metric.ManualReader) (bool, float64) {
	t.Helper()
	...
	for _, scopeMetrics := range metrics.ScopeMetrics {
		for _, m := range scopeMetrics.Metrics {
			if m.Name == "keystore_pool_fill_percentage" {
				gauge, ok := m.Data.(metricdata.Gauge[float64])
				assert.True(t, ok, "Expected metric data to be of type Gauge[float64]")
				if len(gauge.DataPoints) > 0 {
					assert.Len(t, gauge.DataPoints, 1)
					return true, gauge.DataPoints[0].Value
				}
...
func TestKeystorePoolFillPercentageMetric(t *testing.T) {
	db, _, dbConf := testutils.NewTestDB(t, testutils.TestDBConfig{
		CreateDatabase: true,
	})
	dbRepo := sql.NewRepository(db)
	...
	t.Run("fill percentage is reported correctly", func(t *testing.T) {
		time.Sleep(50 * time.Millisecond)

		found, value := getKeystorePoolFillMetric(t, reader)
		assert.True(t, found, "Expected to find keystore_pool_fill_percentage metric")
		assert.InDelta(t, 0.6, value, 0.001, "Expected fill percentage to be 0.6 (3/5)")
	})

	t.Run("fill percentage updates after interval", func(t *testing.T) {
		addToKeystorePool(t, dbRepo, 2)
		time.Sleep(150 * time.Millisecond)

		found, value := getKeystorePoolFillMetric(t, reader)
		assert.True(t, found, "Expected to find keystore_pool_fill_percentage metric")
		assert.InDelta(t, 1.0, value, 0.001, "Expected fill percentage to be 1.0 (5/5) after adding 2 more")
	})
}
