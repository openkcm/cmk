...
type Query struct {
	...
	// RawJoins holds verbatim JOIN clauses that cannot be expressed via JoinCondition.
	RawJoins []string
...
// RawJoin appends a verbatim JOIN clause for cases that cannot be expressed via JoinCondition.
func (q *Query) RawJoin(clause string) *Query {
	q.RawJoins = append(q.RawJoins, clause)
	return q
}
