...
func (c *APIController) GetWorkflows(
	...
	for i, dbWorkflow := range workflows {
		apiWorkflow, err := wfTransform.ToAPI(ctx, *dbWorkflow, false, false, nil, idm) // No eligibility check for list view
		if err != nil {
			return nil, errs.Wrap(apierrors.ErrGetWorkflow, err)
		}
...
func (c *APIController) GetWorkflowByID(ctx context.Context,
	...
	workflow, insufficientApprovers, initiatorIneligible, err := c.Manager.Workflow.GetWorkflowByID(
	...
	if err != nil {
		...
		if errs.IsAnyError(err, manager.ErrCheckWorkflowEligibility) {
			...
			initiatorIneligible = false   // Can't determine, so assume eligible
		} else {
