...
func TestAuthzManager_LoadEntitiesInAllowList(t *testing.T) {
	...
	// This is necessary to simulate the environment where the AuthzManager operates
	// Each tenantID will have its own set of groups with different roles
	// The AuthzManager will then load these groups into its allowlist
	...
	am := manager.NewAuthzManager(t.Context(), repo)
