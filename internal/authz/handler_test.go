...
func TestIsAllowed(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			name: "NoExistentRole",
			entities: []authz.Entity{
				{TenantID: "tenant1", Role: "NonExistentRole", UserGroups: []constants.UserGroup{"Group1"}},
			},
			request: authz.Request{
				User:             authz.User{UserName: "test_user", Groups: []constants.UserGroup{"Group1"}},
		...
		},
		{
			name: "EmptyRequest",
			entities: []authz.Entity{
				{TenantID: "tenant1", Role: constants.TenantAdminRole, UserGroups: []constants.UserGroup{"Group1"}},
			},
			request: authz.Request{
				User:             authz.User{UserName: "test_user", Groups: []constants.UserGroup{"Group1"}},
		...
		},
		{
			name: "EmptyTenant",
			entities: []authz.Entity{
				{TenantID: "tenant1", Role: constants.KeyAdminRole, UserGroups: []constants.UserGroup{"Group1"}},
			},
			request: authz.Request{
				User:             authz.User{UserName: "test_user", Groups: []constants.UserGroup{"Group1"}},
				ResourceTypeName: authz.ResourceTypeKey,
...
func BenchmarkIsAllowed(b *testing.B) {
	...
	request := []struct {
	...
	}{
		{
			...
			request: authz.Request{
				User:             authz.User{UserName: testUsername, Groups: []constants.UserGroup{"Group1"}},
				ResourceTypeName: authz.ResourceTypeKeyConfiguration,
				Action:           authz.ActionRead,
				TenantID:         "tenant2000",
			},
		...
		},
		{
			...
			request: authz.Request{
				User:             authz.User{UserName: testUsername, Groups: []constants.UserGroup{"Group1", "Group2"}},
				ResourceTypeName: authz.ResourceTypeKeyConfiguration,
				Action:           authz.ActionRead,
				TenantID:         "tenant300",
			},
		...
		},
		{
			...
			request: authz.Request{
				User:             authz.User{UserName: testUsername, Groups: []constants.UserGroup{"Groupxz"}},
				ResourceTypeName: authz.ResourceTypeKeyConfiguration,
...
func generateUserGroups() []constants.UserGroup {
	groups := make([]constants.UserGroup, maxGroupCount)

	for i := range maxGroupCount {
		groups[i] = constants.UserGroup(fmt.Sprintf("Group%d", i+1))
	}
...
func createEntitiesForTenant(
	tenantIdx int, allGroups []constants.UserGroup, roleAssigner *RoleAssignment,
) []authz.Entity {
