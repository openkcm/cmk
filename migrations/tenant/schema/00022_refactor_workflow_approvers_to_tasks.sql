...
-- Backward-compat view so old code/rollback can still resolve workflow_approvers
-- This can be dropped in a future release once all references are gone
CREATE VIEW workflow_approvers AS SELECT * FROM workflow_tasks;
...
DROP VIEW IF EXISTS workflow_approvers;
